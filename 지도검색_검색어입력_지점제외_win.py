...
def response_json(json_data):
    global tot_cnt, excluded_branches
    ...
    with open(filename, 'a', newline='', encoding='utf-8-sig') as f:
        ...
        if response.status_code == 200:
            try:
                ...
                filtered_count = 0  # '점'으로 끝나는 업체 수를 카운트
                ...
                for i, item in enumerate(items, 1):
                    ...
                    # 디버깅 로그 추가 (문제 발견용)
                    if '송도점' in name or '아울렛' in name:
                        print(f"확인: '{name}', 끝글자: '{name[-1]}', 끝나는지 여부: {name.endswith('점')}")
                    
                    # '점'으로 끝나는 업체 제외 (공백 제거 후 확인)
                    if name.endswith('점'):
                        filtered_count += 1
                        # 제외된 업체 정보 저장
                ...
                print(f"'점'으로 끝나는 업체 {filtered_count}개 제외됨")                   
...
def print_excluded_branches():
    global excluded_branches
    ...
    print("\n===== 제외된 지점 목록 =====")
    print(f"총 {len(excluded_branches)}개의 지점이 제외되었습니다:")
    
    for i, branch in enumerate(excluded_branches, 1):
        print(f"{i}. {branch['name']} - {branch['address']}")
...
def main():
    ...
    if 1:
        ...
        # 모든 조회가 끝난 후 제외된 지점 목록 출력
        print_excluded_branches()
        
        # 지점 목록을 파일로 저장 (선택적)
        if excluded_branches:
            # search_name 사용 (query_value 대신)
            safe_search_name = search_name.replace(' ', '_')
            excluded_filename = f"지점제외처리_{safe_search_name}_{datetime.now().strftime('%Y%m%d')}.csv"
            with open(excluded_filename, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(['순번', '업체명', '주소'])
                
                for i, branch in enumerate(excluded_branches, 1):
                    writer.writerow([i, branch['name'], branch['address']])
            
            print(f"제외된 지점 목록이 파일로 저장되었습니다: {excluded_filename}")
